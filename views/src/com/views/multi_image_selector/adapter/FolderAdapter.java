package com.views.multi_image_selector.adapter;
...
public class FolderAdapter extends BaseAdapter {
		...
		if (holder != null) {
			if (i == 0) {
				...
				if (mFolders.size() > 0) {
					...
					if (f != null) {
						Picasso.with(mContext).load(new File(f.cover.path)).error(R.drawable.mis_default_error)
								.resizeDimen(R.dimen.mis_folder_cover_size, R.dimen.mis_folder_cover_size).centerCrop()
								.into(holder.cover);
					} else {
	...
	class ViewHolder {
		...
		void bindData(Folder data) {
			...
			if (data.cover != null) {
				// 显示图片
				Picasso.with(mContext).load(new File(data.cover.path)).placeholder(R.drawable.mis_default_error)
						.resizeDimen(R.dimen.mis_folder_cover_size, R.dimen.mis_folder_cover_size).centerCrop()
						.into(cover);
			} else {
